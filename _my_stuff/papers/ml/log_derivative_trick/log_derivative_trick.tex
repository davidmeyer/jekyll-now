...
\begin{flalign}
\nabla_{\theta}J(\theta)           & = \begin{pmatrix}  \frac{ \partial J(\theta)}{\partial \theta_{1}}   \\  
...
\frac{dz}{dy} &=  \frac{1}{p(\mathbf{x};\theta)}  \quad \qquad \qquad \qquad  \:  \mathbin{\#} \frac{\log (X)}{dX} \approx \frac{1}{X} \\
 \frac{dy}{d\theta} &= \frac{d}{d\theta} \; p(\mathbf{x};\theta) = \nabla_\theta p(\mathbf{x};\theta) \;   \mathbin{\#} \text{definition (chain rule, again)}\\
