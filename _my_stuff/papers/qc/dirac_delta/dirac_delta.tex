...
\subsection{Linearity Property of The Laplace Transform}
\bigskip
\noindent
...
\normalfont If $f$ is s piecewise continuous on $[0,\infty)$ and of exponential order $c$ then 
...
[6pt]                                                                                     % get a bit of space
&=& M \frac{e^{(c-s)t}} {c-s} \Big |_0^\infty                          &\quad  \mathrel{\#} \int_0^\infty e^{kx} dx = \frac{1}{k}e^{kx} + C \text{ (ignore $C$, use $u = kt$ so that $du = k dt$...)} \\
[6pt]                                                                                     % get a bit of space     
&=& M \frac{e^{(c-s)\infty}} {c-s} - \frac{Me^{(c-s)0}} {c-s}   &\quad  \mathrel{\#} \text{evaluate at endpoints}                                                                                                                      \\
[6pt]                                                                                     % get a bit of space
&=& 0 - \frac{M} {\mathlarger{c-s}}                                      &\quad  \mathrel{\#} \text{we assumed that $s > c$ so $c -s < 0$ $\Rightarrow$ the first term goes to zero}                          \\    
[6pt]                                                                                     % get a bit of space
...
\bigskip
\noindent
This implies that functions $f(t)$ that do not satisfy the Existence Theorem do not have Laplace Transforms since $\mathcal{L}\{f(t)\}$ doesn't converge.
