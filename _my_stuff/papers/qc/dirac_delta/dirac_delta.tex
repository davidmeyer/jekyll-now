...
[15pt]
&\Rightarrow& \big [ s^2 Y(s) - s y(0) - y^\prime (0) \big ] - \mathcal{L}\{y^\prime\} - 2\mathcal{L}\{y\} = 0             &\mathrel{\#} \text{set $Y(s) = \mathcal{L}\{y\}$ and Section \ref{eqn:f_double_prime}}      \\
[15pt]
...
\bigskip
\noindent
So $\mathcal{L}\{y\} = Y(s) = \mathlarger{\frac{s - 1}{s^2 -s -2}}$. Now we can split $Y(s)$ into two fractions using partial fractions 
and solve for $y(t)$ using the Inverse Laplace Transform:
...
[10pt]
&=& \frac{1}{3} e^{2t} + \mathlarger{\frac{\frac{2}{3}}{s+1}}                                         &\mathrel{\#} \text{$\mathcal{L}^{-1} \Big \{ \frac{1}{s-a} \Big \} = e^{at}$ (Section \ref{sec:inverse_laplace_transform})}  \\
[15pt]
&=& \frac{1}{3} e^{2t} + \frac{2}{3} e^{-t}                                                                      &\mathrel{\#} \text{$\mathcal{L}^{-1} \Big \{ \frac{1}{s-a} \Big \} = e^{at}$ (Section \ref{sec:inverse_laplace_transform})} 
\end{array}
\end{equation*}
