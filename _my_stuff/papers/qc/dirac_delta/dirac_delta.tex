...
\begin{array}{lllll}
\mathcal{L}\{af(t)\} 
&=& \int_0^\infty \! a e^{-st} f(t) dt                 & \mathrel{\#} \text{definition of the Laplace Transform (Definition \ref{def:laplace_transform})}   \\  
[6pt]                                                                                 % get a bit of space
...
\bigskip
\noindent
So $F^\prime(s) = -\mathcal{L}\{tf(t)\}$. 

\bigskip
\noindent
We can also pretty easily see that in general $F^{(n)}(s) = (-1)^n \mathcal{L}\{t^n f(t)\}$.
