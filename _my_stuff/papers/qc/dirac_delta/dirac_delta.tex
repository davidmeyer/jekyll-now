...
\begin{array}{lllll}
\mathcal{L}\{\delta_a(t)\} 
&=& \int_0^\infty  e^{-st} \delta_a(t) dt    &\quad  \mathrel{\#} \text{definition of the Laplace Transform} \\  
...
\bigskip
\noindent
Note that if $s = c$ then $\frac{M}{\mathlarger{s-c}}$ does not converge and $\mathcal{L}\{f(t)\}$ does not exist. Similarly, if $c > s$ then $\lim\limits_{d \to \infty}\Big [\frac{M}{c-s} e^{(c-s)d} \Big ]$ 
does not converge and $\mathcal{L}\{f(t)\}$ does not exist. All of this implies that functions $f(t)$ that do not satisfy the conditions of the Existence Theorem do not have Laplace Transforms.
