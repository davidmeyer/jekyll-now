...
\noindent
Fermat's Christmas Theorem (aka Fermat's Theorem on sums of two squares) states that any odd prime number $p$ can be expressed as
...
\noindent
where $r,s \in \mathbb{N}$, if and only if $p$ is congruent to  $1$ modulo $4$.
